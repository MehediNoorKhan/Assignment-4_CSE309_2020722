...
$api_key = "247d2898a61c28541b150bedd751fabe";
$units = "metric";

function getWeatherData($location)
{
    global $api_key, $units;

    $url = "https://api.openweathermap.org/data/2.5/forecast?q=$location&appid=$api_key&units=$units";
...
if (isset($_GET['new_location'])) {
    if ($_GET['new_location'] == "") {
        $data = getWeatherData("Dhaka");
    } else {
        $new_location = urlencode($_GET['new_location']);
        $data = getWeatherData($new_location);
...
$cities = array("Dhaka,bd");

foreach ($cities as $city) {
    $other_data[$city] = getWeatherData($city);
}
